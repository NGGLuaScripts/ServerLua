...
local BROADCAST_INTERVAL = 1 -- Her 1 saniyede bir yayın yap
...
function broadcastScore(driverName, score)
    ac.debug("Skor yayınlanıyor: " .. driverName .. " - " .. score)
    ac.sendChatMessage("SCORE|" .. driverName .. "|" .. math.floor(score))
end

function script.onChatMessage(message)
    if message:find("SCORE|") then
        local parts = {}
        for part in message:gmatch("[^|]+") do
            table.insert(parts, part)
        end
        
        if #parts == 3 and parts[1] == "SCORE" then
            local driverName = parts[2]
            local score = tonumber(parts[3])
            
            if driverName and score then
                -- Skoru güncelle
                if not personalBestScores[driverName] or score > personalBestScores[driverName] then
                    personalBestScores[driverName] = score
                    ac.debug("Skor güncellendi: " .. driverName .. " - " .. score)
                end
            end
        end
    end
...
function script.update(dt)
    ...
    lastBroadcastTime = lastBroadcastTime + dt
    if lastBroadcastTime >= BROADCAST_INTERVAL then
        if personalBestScore > 0 then
            broadcastScore(playerID, personalBestScore)
        end
        lastBroadcastTime = 0
    end
...
function updateLeaderboard()
    ...
    for i = 0, maxPlayers - 1 do
        ...
        if car and car.isConnected and driverName and driverName ~= "" and driverName ~= "function: 0xff" then
            ...
            -- Debug mesajları
            ac.debug("Liderlik Tablosu - Oyuncu: " .. driverName)
            ac.debug("Skor: " .. (personalBestScores[driverName] or 0))
