    ...
    for i = 0, maxPlayers - 1 do
        ...
        if car and car.isConnected and driverName and driverName ~= "" then
            ...
            if driftScore > 0 then
                -- Mevcut drift skorunu güncelle
                playerDriftScores[driverName] = (playerDriftScores[driverName] or 0) + driftScore
                
                -- Eğer bu lokal oyuncu ise, currentRunScore'u güncelle
                if driverName == playerID then
                    currentRunScore = playerDriftScores[driverName]
                end
                
                -- Kişisel en iyi skoru güncelle
                if playerDriftScores[driverName] > (personalBestScores[driverName] or 0) then
                    personalBestScores[driverName] = math.floor(playerDriftScores[driverName])
                    
                    -- Eğer bu lokal oyuncu ise, personalBestScore'u da güncelle
                    if driverName == playerID then
                        personalBestScore = personalBestScores[driverName]
                    end
                end
            end
        end
...
function updateLeaderboard()
    ...
    for i = 0, maxPlayers - 1 do
        ...
        if car and car.isConnected and driverName and driverName ~= "" and driverName ~= "function: 0xff" then
            -- Oyuncunun mevcut drift skoru ve kişisel en iyi skorunu kontrol et
            local currentScore = playerDriftScores[driverName] or 0
            local bestScore = personalBestScores[driverName] or 0
            
            -- En yüksek skoru kullan
            local finalScore = math.max(currentScore, bestScore)
