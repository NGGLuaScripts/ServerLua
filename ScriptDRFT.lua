    ...
    for i = 0, maxPlayers - 1 do
        ...
        if car and car.isConnected and driverName and driverName ~= "" then
            ...
            if driftScore > 0 then
                ...
                if not driftScores[driverName] then
                    driftScores[driverName] = 0
                end
                driftScores[driverName] = driftScores[driverName] + driftScore
                
                -- Kişisel en iyi skoru güncelle
                if driftScores[driverName] > personalBestScores[driverName] then
                    personalBestScores[driverName] = math.floor(driftScores[driverName])
                end
            end
        end
    end
...
function updateLeaderboard()
    ...
    for i = 0, maxPlayers - 1 do
        ...
        if car and car.isConnected and driverName and driverName ~= "" and driverName ~= "function: 0xff" then
            -- Eğer oyuncunun kişisel en iyi skoru yoksa 0 olarak başlat
            if not personalBestScores[driverName] then
                personalBestScores[driverName] = 0
            end
